...
const ALLOWED_BLOOD: BloodGroup[] = ["A+", "A-", "B+", "B-", "AB+", "AB-", "0+", "0-"];
...
export async function createPatient(data: Omit<Patient, "_id" | "createdAt" | "updatedAt">): Promise<Patient | null> {
  if (!data.fullName || !data.dateOfBirth || !data.identificationNumber || !data.recordNumber || !data.gender) {
  ...
  const exists = await c.findOne({
    $or: [{ identificationNumber: data.identificationNumber }, { recordNumber: data.recordNumber }],
  });

  if (exists) {
    throw new Error("A patient with the same identificationNumber or recordNumber already exists");
  }
...
export async function findPatientById(id: string | ObjectId): Promise<Patient | null> {
